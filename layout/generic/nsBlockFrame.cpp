...
bool nsBlockFrame::DrainOverflowLines() {
  ...
  if (prevBlock) {
    ...
    if (overflowLines) {
      ...
      delete overflowLines;
      AddFrames(ocContinuations, mFrames.LastChild(), Nothing());
...
void nsBlockFrame::AppendFrames(ChildListID aListID, nsFrameList& aFrameList) {
  ...
  AddFrames(aFrameList, lastKid, Nothing());
...
void nsBlockFrame::InsertFrames(
...
void nsBlockFrame::AddFrames(nsFrameList& aFrameList, nsIFrame* aPrevSibling,
                             Maybe<nsLineList::iterator> aPrevSiblingLine) {
  ...
  if (aPrevSiblingLine.isSome()) {
    prevSibLine = aPrevSiblingLine.value();
    FrameLines* overflowLines = GetOverflowLines();
    ...
    if (overflowLines) {
      ...
      // *if* we have overflow lines, that are primary lines aren't
...
void nsBlockFrame::SetInitialChildList(ChildListID aListID,
  ...
  if (kFloatList == aListID) {
    ...
    AddFrames(aChildList, nullptr, Nothing());
  } else {
