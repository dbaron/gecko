...
void ReflowInput::InitResizeFlags(nsPresContext* aPresContext,
    ...
    mFlags.mBResizeForPercentages =
        !wm.IsOrthogonalTo(mCBReflowInput->mWritingMode)
            ...
            : mCBReflowInput->IsIResize();
             ...
             (!wm.IsOrthogonalTo(mCBReflowInput->mWritingMode)
                  // Note: This causes perf regression on F52 test case, but
                  // fixes correctness issue in bug1351924.html
                  //
  ...
  // When we have an nsChangeHint_UpdateComputedBSize, we'll set a bit on the
  // frame to indicate we're resizing. Here we note that, and propagate it
  // forward for cases where we have children with percentage sizes, as those
  // will need to resize too and may not have the appropriate bits set for
  // themselves. In particular we need to propagate across a display type
  // boundary. Also tricky case is switching from an auto to a non-auto size
  // with percent children, as percent sizes inherit auto if the parent is auto.
