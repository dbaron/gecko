...
static nsresult CreateContinuation(nsIFrame* aFrame,
  ...
  *aNewFrame = presShell->FrameConstructor()->CreateContinuingFrame(
  ...
  parent->InsertFrames(nsIFrame::kNoReflowPrincipalList, aFrame, parentLine, temp);
    ...
    if (frame == NS_BIDI_CONTROL_FRAME) {
      ...
      if (isTextFrame) {
        ...
        if ((runLength > 0) && (runLength < fragmentLength)) {
          ...
          rv = EnsureBidiContinuation(frame, currentLine.value(), &nextBidi,
          ...
          // ResolveParagraphWithinBlock in the middle of
          // TraverseFrames.  If so, once that's done, we're going to
          // continue on with TraverseFrames.
          //
          // TraverseFrames uses aBpd->mPrevFrame as an optimization to
          // help advance its line iterator correctly.  The way this
          // optimization works relies on the invariant that mPrevFrame
          // is within the line pointed to by the line iterator.
          //
            ...
            do {
              ...
              if (uint32_t(endLine) < text.Length()) {
                ...
                if (!next) {
                  ...
                  CreateContinuation(frame, aLineIter->GetLine(), &next, true);
